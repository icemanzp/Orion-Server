    ...
    static {
        ...
        try {
            ...
            ks.load(SecureChatKeyStore.asInputStream(
                    ServerConfigWrappar.getNettyServerInfo().getBaseInfo().getSshInfo().getKeystorePath()),
                    //jks password
                    SecureChatKeyStore.getKeyStorePassword(
                            ServerConfigWrappar.getNettyServerInfo().getBaseInfo().getSshInfo().getKeystorePasswd()));

            // Set up key manager factory to use our key store
            KeyManagerFactory kmf = KeyManagerFactory.getInstance(algorithm);
            kmf.init(ks, SecureChatKeyStore.getCertificatePassword(
                    ServerConfigWrappar.getNettyServerInfo().getBaseInfo().getSshInfo().getKeystoreCertpasswd()));
